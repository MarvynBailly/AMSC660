function mnist_2categories_quadratic_NLLS(method)
close all
fsz = 20;
...
batchSize = 1000;
kmax = 3e3; % the max number of iterations
tol = 1e-3;

if method == 1
    [w,f,gnorm,k] = LevenbergMarquardt(r_and_J,w,kmax,tol);
elseif method == 2
    [w,f,gnorm,k] = SG(fun,gfun,w,kmax,tol,batchSize,Ntrain);
elseif method == 3
    [w,f,gnorm,k] = nesterov(fun,gfun,w,kmax,tol,batchSize,Ntrain);
elseif method == 4
    [w,f,gnorm,k] = adams(fun,gfun,w,kmax,tol,batchSize,Ntrain);
end
fprintf("Method converged on iteration %d\n",k)
...
if method == 1
    ...
    plot((1:k) ./ batchSize, f,'Linewidth',2);
    xlabel('epoch','fontsize',fsz);
    ylabel('f','fontsize',fsz);
    set(gca,'fontsize',fsz,'Yscale','log');
    figure;
    plot((1:k) ./ batchSize,gnorm,'Linewidth',2);
    xlabel('epoch','fontsize',fsz);
    ylabel('||g||','fontsize',fsz);
    set(gca,'fontsize',fsz,'Yscale','log');
end
...
f = sum(log(1 + exp(-myquadratic1(Xtrain,label,I,w))))/length(I) + 0.5*lam*w'*w;
end
%%
function g = qlossgrad(I,Xtrain,label,w,lam)
aux = exp(-myquadratic1(Xtrain,label,I,w));
a = -aux./(1+aux);
X = Xtrain(I,:);
d = size(X,2);
d2 = d^2;
...
%%
function q = myquadratic1(Xtrain,label,I,w)
X = Xtrain(I,:);
d = size(X,2);
d2 = d^2;
y = label(I);
